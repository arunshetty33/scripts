...
aws lambda list-functions --profile $aws_profile --region $region --query 'Functions[].[FunctionName,Runtime]' --output text | grep $check_runtime | awk '{print $1}' 2>&1 | tee $file
